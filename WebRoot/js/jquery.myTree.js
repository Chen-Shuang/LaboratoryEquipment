	...
	$.fn.extend({ // 对于生成二级目录的树，最少需要两次遍历 
		toTree : function(jsonData, right_parent_code) {
			...
			$.each(jsonData, function(i, v) { // 第一次遍历，取出一级菜单，id属性值为"o"+id
				if (v.parent_id == right_parent_code) { // 如果为一级菜单
					$li_node = $("<li class='me'></li>"); // 创建li节点
					
					$ul = $("<ul class='submenu'></ul>");
					$ul.attr("id", "o" + v.id); // 给li节点添加id，子菜单通过此id向此追加
					
					var $i_node = $("<a href='" + "#" + v.uri + "' class='dropdown-toggle'><i class='menu-icon fa "+v.s_class+"'></i><span class='menu-text' style='font-size:14px'>" + v.privilege_name + "</span></a>"); // 菜单内容及其链接
			...
			$.each(jsonData, function(i, v) { // 第二次遍历，取出二级菜单，添加到id属性值为"o"+pId的节点下
				if (v.parent_id != right_parent_code) {
					var $i_node = $("<li style='list-style-type:none; font-size:13px;'><a href='" + v.uri + "'><i class='" + v.s_class +"'></i>" + v.privilege_name + "</a></li>");
					$i_node.attr("id", "r" + v.id);
					$("#" + "o" + v.parent_id).append($i_node);
				}
			});
		}
	});
