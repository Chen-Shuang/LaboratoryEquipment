...
public class ActController extends Controller {
	...
	public void getMenuList() {	
		...
		if(user.get("rank").equals(1)){
			dataList = Db.find("select * from memu where rank=1 order by id"); // 普通用户权限
		}else if(user.get("rank").equals(2)){
			dataList = Db.find("select * from memu where rank!=0 order by id"); // 领导用户权限
		}else{
			dataList = Db.find("select * from memu order by id"); // 超级管理员权限
		}
